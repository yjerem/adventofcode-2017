...
DATA.readlines.each do |inst|
  if inst =~ /^(\w+) (inc|dec) (-?\d+) if (\w+) ([<=>!]+) (-?\d+)$/
    reg, op, arg, cond_reg, cond_op, cond_arg = $1, $2, $3.to_i, $4, $5, $6.to_i
    cond_result =
      case cond_op
      when ">" then mem[cond_reg] > cond_arg
      when "<" then mem[cond_reg] < cond_arg
      when ">=" then mem[cond_reg] >= cond_arg
      when "<=" then mem[cond_reg] <= cond_arg
      when "==" then mem[cond_reg] == cond_arg
      when "!=" then mem[cond_reg] != cond_arg
        ...
        puts "unhandled cond_op #{cond_op}"
      end
    if cond_result
      case op
      when "inc" then mem[reg] += arg
      when "dec" then mem[reg] -= arg
      else
        puts "unhandled op #{op}"
      end
    end
    maxes.push mem.values.max
  elsif inst.strip.length > 0
    puts "unhandled instruction: inst"
  end
end
