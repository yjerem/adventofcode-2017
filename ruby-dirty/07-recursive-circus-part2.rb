Node = Struct.new(:name, :weight, :parent, :children, :total_weight)

nodes = {}

DATA.readlines.each do |line|
  if line =~ /^(\w+) \((\d+)\)( -> .+)?$/
    node = Node.new($1, $2.to_i, nil, $3.to_s[4..-1].to_s.split(', '), nil)
    if nodes[node.name]
      nodes[node.name].weight = node.weight
      nodes[node.name].children = node.children
    else
    ...
    node.children.each do |child|
      if nodes[child]
        ...
        nodes[child] = Node.new(child, nil, node.name, nil, nil)
      end
    end
  end
end

def find_total_weights(nodes, name)
  nodes[name].total_weight =
    nodes[name].weight +
    nodes[name].children.map { |child| find_total_weights(nodes, child) }.sum
end
...
find_total_weights(nodes, root)

nodes.each do |name, node|
  weights = {}
  node.children.each do |child|
    weights[nodes[child].total_weight] ||= 0
    weights[nodes[child].total_weight] += 1
  end
  if weights.length > 1
    unbalanced = weights.to_a.min_by { |_, n| n }[0]
    proper_weight = weights.to_a.max_by { |_, n| n }[0]
    node.children.each do |child|
      if nodes[child].total_weight == unbalanced
        nodes[child].weight += proper_weight - unbalanced
        p nodes[child].weight
        exit
      end
    end
  end
